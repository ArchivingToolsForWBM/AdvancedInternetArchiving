	...
	async function GetTwitterLinks() {
		...
		//Get twitter URLs the browser is on that have potentially missed URLs, such as "Something went wrong. Try reloading."
			...
			for (let i = 0; i < document.getElementsByTagName("SPAN").length && SomethingWentWrong == false; i++ ) {
				if(document.getElementsByTagName("SPAN")[i].innerText == "Something went wrong. Try reloading."||document.getElementsByTagName("SPAN")[i].innerText == "Looks like you lost your connection. Please check it and try again."||document.getElementsByTagName("SPAN")[i].innerText == "Something went wrong, but don’t fret — it’s not your fault.") {
